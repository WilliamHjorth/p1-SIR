...
// Struktur til at holde styr på rejsende (fra anden metode)
typedef struct {
    float S;
    float E;
    float I;
...
// Struktur til at holde styr på rejsende for hver aldersgruppe
typedef struct {
    Rejsende rejsende_AAL_til_KBH[ALDERS_GRUPPER];
    Rejsende rejsende_KBH_til_AAL[ALDERS_GRUPPER];
    Rejsende lang_ophold_infected_AAL_til_KBH[ALDERS_GRUPPER];
    Rejsende lang_ophold_infected_KBH_til_AAL[ALDERS_GRUPPER];
} RejsendeGrupper;
...
void sirModelToByer(int model_type, int use_app, int use_vaccine, int city_choice, FILE *file, int replicate_num, int is_stochastic, int print_to_terminal)
{
    ...
    // Initialiser rejsende strukturer (fra anden metode)
    RejsendeGrupper rejsende;
    ...
    for (int i = 0; i < ALDERS_GRUPPER; i++)
    {
        ...
        // Nulstil rejsende strukturer
        rejsende.rejsende_AAL_til_KBH[i] = (Rejsende){0, 0, 0, 0, 0};
        rejsende.rejsende_KBH_til_AAL[i] = (Rejsende){0, 0, 0, 0, 0};
        rejsende.lang_ophold_infected_AAL_til_KBH[i] = (Rejsende){0, 0, 0, 0, 0};
        rejsende.lang_ophold_infected_KBH_til_AAL[i] = (Rejsende){0, 0, 0, 0, 0};
    }
    ...
    for (int n = 0; n < MAX_DAYS; n++)
    {
        if (is_stochastic)
        {
            ...
            for (int step = 0; step < STEPS_PER_DAY; step++)
            {
                ...
                for (int i = 0; i < ALDERS_GRUPPER; i++)
                {
                    ...
                    // 1. Beregn hvor mange der rejser fra hver by (0,1% af hver kategori) - skaleret med dt
                    float S_ud_AAL = t * dt * S_AAL[i];
                    float E_ud_AAL = t * dt * E_AAL[i];
                    float I_ud_AAL = t * dt * I_AAL[i];
                    float R_ud_AAL = t * dt * R_AAL[i];

                    float S_ud_KBH = t * dt * S_KBH[i];
                    float E_ud_KBH = t * dt * E_KBH[i];
                    float I_ud_KBH = t * dt * I_KBH[i];
                    float R_ud_KBH = t * dt * R_KBH[i];
                    ...
                    S_KBH[i] += S_ud_AAL;
                    E_KBH[i] += E_ud_AAL;
                    I_KBH[i] += I_ud_AAL_normal;  // Kun 10% af inficerede kommer med det samme
                    R_KBH[i] += R_ud_AAL;

                    // KBH -> AAL: Alle normale rejsende + kun 10% af inficerede
                    S_AAL[i] += S_ud_KBH;
                    E_AAL[i] += E_ud_KBH;
                    I_AAL[i] += I_ud_KBH_normal;  // Kun 10% af inficerede kommer med det samme
                    R_AAL[i] += R_ud_KBH;

                    // 5. Gem de rejsende der skal håndteres særskilt
                    rejsende.rejsende_AAL_til_KBH[i].S += S_ud_AAL;
                    rejsende.rejsende_AAL_til_KBH[i].E += E_ud_AAL;
                    rejsende.rejsende_AAL_til_KBH[i].I += I_ud_AAL_normal;  // Kun de 10% der rejser normalt
                    rejsende.rejsende_AAL_til_KBH[i].R += R_ud_AAL;
                    rejsende.rejsende_AAL_til_KBH[i].dage_i_byen++;

                    rejsende.rejsende_KBH_til_AAL[i].S += S_ud_KBH;
                    rejsende.rejsende_KBH_til_AAL[i].E += E_ud_KBH;
                    rejsende.rejsende_KBH_til_AAL[i].I += I_ud_KBH_normal;  // Kun de 10% der rejser normalt
                    rejsende.rejsende_KBH_til_AAL[i].R += R_ud_KBH;
                    rejsende.rejsende_KBH_til_AAL[i].dage_i_byen++;

                    // Gem de 90% inficerede der bliver længere
                    rejsende.lang_ophold_infected_AAL_til_KBH[i].I += I_ud_AAL_long_stay;
                    rejsende.lang_ophold_infected_AAL_til_KBH[i].dage_i_byen++;

                    rejsende.lang_ophold_infected_KBH_til_AAL[i].I += I_ud_KBH_long_stay;
                    rejsende.lang_ophold_infected_KBH_til_AAL[i].dage_i_byen++;

                    // 6. Håndter tilbagevendende rejsende efter 3 dage
                    if (rejsende.rejsende_AAL_til_KBH[i].dage_i_byen >= 3 && 
                        (rejsende.rejsende_AAL_til_KBH[i].S > 0 || rejsende.rejsende_AAL_til_KBH[i].E > 0 || 
                         rejsende.rejsende_AAL_til_KBH[i].I > 0 || rejsende.rejsende_AAL_til_KBH[i].R > 0)) {
                        // AAL -> KBH rejsende vender tilbage til AAL
                        S_AAL[i] += rejsende.rejsende_AAL_til_KBH[i].S;
                        E_AAL[i] += rejsende.rejsende_AAL_til_KBH[i].E;
                        I_AAL[i] += rejsende.rejsende_AAL_til_KBH[i].I;
                        R_AAL[i] += rejsende.rejsende_AAL_til_KBH[i].R;
                        
                        // Træk fra KBH
                        S_KBH[i] -= rejsende.rejsende_AAL_til_KBH[i].S;
                        E_KBH[i] -= rejsende.rejsende_AAL_til_KBH[i].E;
                        I_KBH[i] -= rejsende.rejsende_AAL_til_KBH[i].I;
                        R_KBH[i] -= rejsende.rejsende_AAL_til_KBH[i].R;
                        
                        // Nulstil
                        rejsende.rejsende_AAL_til_KBH[i] = (Rejsende){0, 0, 0, 0, 0};
                    }
                    
                    if (rejsende.rejsende_KBH_til_AAL[i].dage_i_byen >= 3 && 
                        (rejsende.rejsende_KBH_til_AAL[i].S > 0 || rejsende.rejsende_KBH_til_AAL[i].E > 0 || 
                         rejsende.rejsende_KBH_til_AAL[i].I > 0 || rejsende.rejsende_KBH_til_AAL[i].R > 0)) {
                        // KBH -> AAL rejsende vender tilbage til KBH
                        S_KBH[i] += rejsende.rejsende_KBH_til_AAL[i].S;
                        E_KBH[i] += rejsende.rejsende_KBH_til_AAL[i].E;
                        I_KBH[i] += rejsende.rejsende_KBH_til_AAL[i].I;
                        R_KBH[i] += rejsende.rejsende_KBH_til_AAL[i].R;
                        
                        // Træk fra AAL
                        S_AAL[i] -= rejsende.rejsende_KBH_til_AAL[i].S;
                        E_AAL[i] -= rejsende.rejsende_KBH_til_AAL[i].E;
                        I_AAL[i] -= rejsende.rejsende_KBH_til_AAL[i].I;
                        R_AAL[i] -= rejsende.rejsende_KBH_til_AAL[i].R;
                    ...
                    if (rejsende.lang_ophold_infected_AAL_til_KBH[i].dage_i_byen >= 7 && rejsende.lang_ophold_infected_AAL_til_KBH[i].I > 0) {
                        // De inficerede fra AAL i KBH vender tilbage til AAL (nu som recovered)
                        R_AAL[i] += rejsende.lang_ophold_infected_AAL_til_KBH[i].I;
                        I_KBH[i] -= rejsende.lang_ophold_infected_AAL_til_KBH[i].I;  // Træk fra KBH's inficerede
                        
                        // Nulstil
                        rejsende.lang_ophold_infected_AAL_til_KBH[i] = (Rejsende){0, 0, 0, 0, 0};
                    }
                    
                    if (rejsende.lang_ophold_infected_KBH_til_AAL[i].dage_i_byen >= 7 && rejsende.lang_ophold_infected_KBH_til_AAL[i].I > 0) {
                        // De inficerede fra KBH i AAL vender tilbage til KBH (nu som recovered)
                        R_KBH[i] += rejsende.lang_ophold_infected_KBH_til_AAL[i].I;
                        I_AAL[i] -= rejsende.lang_ophold_infected_KBH_til_AAL[i].I;  // Træk fra AAL's inficerede
                    ...
                    float N_AAL_current = S_AAL[i] + E_AAL[i] + I_AAL[i] + R_AAL[i] + H_AAL[i];
                    float N_KBH_current = S_KBH[i] + E_KBH[i] + I_KBH[i] + R_KBH[i] + H_KBH[i];

                    // 9. Differentialligninger for SEIR-modellen - AALBORG
                    double rate_inf_A = beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current;
                    ...
                    if (n_inf_A > (long)S_AAL[i]) n_inf_A = (long)S_AAL[i];
                    if (n_prog_A > (long)E_AAL[i]) n_prog_A = (long)E_AAL[i];
                    if (n_rec_A > (long)I_AAL[i]) n_rec_A = (long)I_AAL[i];
                    if (n_hosp_A > (long)I_AAL[i]) n_hosp_A = (long)I_AAL[i];
                    if (n_h_rec_A > (long)H_AAL[i]) n_h_rec_A = (long)H_AAL[i];
                    ...
                    double rate_inf_K = beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current;
                    ...
                    // Sikre at vi ikke får negative værdier
                    if (n_inf_K > (long)S_KBH[i]) n_inf_K = (long)S_KBH[i];
                    if (n_prog_K > (long)E_KBH[i]) n_prog_K = (long)E_KBH[i];
                    if (n_rec_K > (long)I_KBH[i]) n_rec_K = (long)I_KBH[i];
                    if (n_hosp_K > (long)I_KBH[i]) n_hosp_K = (long)I_KBH[i];
                    if (n_h_rec_K > (long)H_KBH[i]) n_h_rec_K = (long)H_KBH[i];
        ...
        {
            // Deterministiske flows med ny transferrate metode
            ...
            for (int i = 0; i < ALDERS_GRUPPER; i++)
            {
                ...
                float S_ud_AAL = t * S_AAL[i];
                float E_ud_AAL = t * E_AAL[i];
                float I_ud_AAL = t * I_AAL[i];
                float R_ud_AAL = t * R_AAL[i];

                float S_ud_KBH = t * S_KBH[i];
                float E_ud_KBH = t * E_KBH[i];
                float I_ud_KBH = t * I_KBH[i];
                float R_ud_KBH = t * R_KBH[i];
                ...
                // Opdater N-værdierne (vigtigt for differentialligningerne)
                float N_AAL_current = S_AAL[i] + E_AAL[i] + I_AAL[i] + R_AAL[i] + H_AAL[i];
                float N_KBH_current = S_KBH[i] + E_KBH[i] + I_KBH[i] + R_KBH[i] + H_KBH[i];

                if (model_type == 1)
                { // SIR
                    dS_AAL[i] = -beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current;
                    dI_AAL[i] = beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current - gamma_i_A * I_AAL[i];
                    dR_AAL[i] = gamma_i_A * I_AAL[i];
                    dE_AAL[i] = 0;
                    dH_AAL[i] = 0;

                    dS_KBH[i] = -beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current;
                    dI_KBH[i] = beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current - gamma_i_K * I_KBH[i];
                ...
                { // SEIR
                    dS_AAL[i] = -beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current;
                    dE_AAL[i] = beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current - sigma_i_A * E_AAL[i];
                    dI_AAL[i] = sigma_i_A * E_AAL[i] - gamma_i_A * I_AAL[i];
                    dR_AAL[i] = gamma_i_A * I_AAL[i];
                    dH_AAL[i] = 0;

                    dS_KBH[i] = -beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current;
                    dE_KBH[i] = beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current - sigma_i_K * E_KBH[i];
                ...
                { // SEIHR
                    dS_AAL[i] = -beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current + Modtagelighed_i_A * R_AAL[i];
                    dE_AAL[i] = beta_i_A * S_AAL[i] * total_I_AAL / N_AAL_current - sigma_i_A * E_AAL[i];
                    dI_AAL[i] = sigma_i_A * E_AAL[i] - gamma_i_A * I_AAL[i] - h_i_A * I_AAL[i];
                    dH_AAL[i] = h_i_A * I_AAL[i] - gamma_i_A / 2 * H_AAL[i];
                    dR_AAL[i] = gamma_i_A * I_AAL[i] + gamma_i_A / 2 * H_AAL[i] - Modtagelighed_i_A * R_AAL[i];

                    dS_KBH[i] = -beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current + Modtagelighed_i_A * R_KBH[i];
                    dE_KBH[i] = beta_i_K * S_KBH[i] * total_I_KBH / N_KBH_current - sigma_i_K * E_KBH[i];
                    dI_KBH[i] = sigma_i_K * E_KBH[i] - gamma_i_K * I_KBH[i] - h_i_K * I_KBH[i];
                    dH_KBH[i] = h_i_K * I_KBH[i] - gamma_i_K / 2 * H_KBH[i];
                    dR_KBH[i] = gamma_i_K * I_KBH[i] + gamma_i_K / 2 * H_KBH[i] - Modtagelighed_i_A * R_KBH[i];
                }
