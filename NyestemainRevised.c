...
void sirModelToByer(int model_type, int use_app, int use_vaccine, FILE *file, int replicate_num, int print_to_terminal);
void runMultipleReplicates(int model_type, int use_app, int use_vaccine, int numReplicates);
...
void createGnuplotScript(const char *scriptFile, const char *dataFile, int numReplicates, int model_type);
void createGnuplotScriptSingle(const char *scriptFile, const char *dataFile, int model_type);
...
void bruger_input()
{
    ...
    else if (ch == 'M' || ch == 'm')
    {
        ...
        if (sim_type == 'N' || sim_type == 'n')
        {
            ...
            sirModelToByer(model_type, use_app, use_vaccine, file, 0, 1);
            fclose(file);
            
            printf("Simulering færdig. Data gemt i data_file.txt\n");
            
            // Create gnuplot script for single simulation
            createGnuplotScriptSingle("plot_single.gnu", "data_file.txt", model_type);
            printf("Gnuplot script gemt i plot_single.gnu\n");
            printf("Kør: gnuplot plot_single.gnu\n");
        }
        else if (sim_type == 'S' || sim_type == 's')
        {
            // Stochastic simulation with multiple replicates
            int numReplicates;
            ...
            runMultipleReplicates(model_type, use_app, use_vaccine, numReplicates);
        }
        else
        {
...
void tilpas_funktion()
{
    ...
    sirModelToByer(model_type, 0, 0, file, 0, 1);
    fclose(file);
...
void sirModelToByer(int model_type, int use_app, int use_vaccine, FILE *file, int replicate_num, int print_to_terminal)
{
    ...
    // Write two newlines to separate replicates for Gnuplot
    if (file != NULL)
    {
        fprintf(file, "\n\n");
    }
...
void runMultipleReplicates(int model_type, int use_app, int use_vaccine, int numReplicates)
{
    ...
    for (int rep = 0; rep < numReplicates; rep++)
    {
        ...
        sirModelToByer(model_type, use_app, use_vaccine, file, rep, 0);
    }
    ...
    createGnuplotScript("plot_replicates.gnu", "stochastic_replicates.txt", numReplicates, model_type);
...
void createGnuplotScript(const char *scriptFile, const char *dataFile, int numReplicates, int model_type)
{
    ...
    fprintf(fp, "set grid\n");
    fprintf(fp, "set key outside right\n");
    ...
    if (model_type == 1) {
        // SIR: S, I, R
        fprintf(fp, "plot for [i=0:%d] '%s' index i using 1:2 with lines ls 1 title 'S', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:4 with lines ls 3 title 'I', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:6 with lines ls 5 title 'R'\n", numReplicates-1, dataFile);
    } else if (model_type == 2) {
        // SEIR: S, E, I, R
        fprintf(fp, "plot for [i=0:%d] '%s' index i using 1:2 with lines ls 1 title 'S', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:3 with lines ls 2 title 'E', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:4 with lines ls 3 title 'I', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:6 with lines ls 5 title 'R'\n", numReplicates-1, dataFile);
    } else if (model_type == 3) {
        // SEIHR: S, E, I, H, R
        ...
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:3 with lines ls 2 title 'E', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:4 with lines ls 3 title 'I', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:5 with lines ls 4 title 'H', \\\n", numReplicates-1, dataFile);
        fprintf(fp, "     for [i=0:%d] '%s' index i using 1:6 with lines ls 5 title 'R'\n", numReplicates-1, dataFile);
    }
...
void Gnuplot(int modeltype)
{
    // This function can be implemented to call gnuplot directly
    // For now, it's a placeholder
}

// Create Gnuplot script for single simulation (normal/deterministic)
void createGnuplotScriptSingle(const char *scriptFile, const char *dataFile, int model_type)
{
    FILE *fp = fopen(scriptFile, "w");
    if (fp == NULL)
    ...
    // Define colors (solid for single simulation)
    ...
    // Plot based on model type (Aalborg data: columns 2-6)
    if (model_type == 1) {
        // SIR: S, I, R (columns 2, 4, 6)
        fprintf(fp, "plot '%s' using 1:2 with lines ls 1 title 'S (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:4 with lines ls 3 title 'I (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:6 with lines ls 5 title 'R (Aalborg)'\n", dataFile);
    } else if (model_type == 2) {
        // SEIR: S, E, I, R (columns 2, 3, 4, 6)
        fprintf(fp, "plot '%s' using 1:2 with lines ls 1 title 'S (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:3 with lines ls 2 title 'E (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:4 with lines ls 3 title 'I (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:6 with lines ls 5 title 'R (Aalborg)'\n", dataFile);
    } else if (model_type == 3) {
        // SEIHR: S, E, I, H, R (columns 2, 3, 4, 5, 6)
        fprintf(fp, "plot '%s' using 1:2 with lines ls 1 title 'S (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:3 with lines ls 2 title 'E (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:4 with lines ls 3 title 'I (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:5 with lines ls 4 title 'H (Aalborg)', \\\n", dataFile);
        fprintf(fp, "     '%s' using 1:6 with lines ls 5 title 'R (Aalborg)'\n", dataFile);
    }
