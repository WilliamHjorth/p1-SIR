...
int sirModelToByer(float S_AAL, float I_AAL, float R_AAL, float S_KBH, float I_KBH, float R_KBH);
...
int main(void)
{
    ...
    sirModelToByer(S0_AAL, I0_AAL, R0_AAL, S0_KBH, I0_KBH, R0_KBH);
    return 0;
}

// funktion der plotter en sir model for to byer ud fra givne startværdier
int sirModelToByer(float S_AAL_start, float I_AAL_start, float R_AAL_start, float S_KBH_start, float I_KBH_start, float R_KBH_start) // kun til windows indtilvidere
{
    ...
    for (int n = 0; n < 150; n++)
    {
        ...
        printf("Day %d: AAL(S=%.0f, I=%.0f, R=%.0f) KBH(S=%.0f, I=%.0f, R=%.0f)\n", 
               n, S_AAL, I_AAL, R_AAL, S_KBH, I_KBH, R_KBH);
    ...
    fprintf(pipe, "     'data_file.txt' using 1:3 with lines lw 2 title 'AAL Infected', \\\n");
    fprintf(pipe, "     'data_file.txt' using 1:4 with lines lw 2 title 'AAL Recovered', \\\n");
    fprintf(pipe, "     'data_file.txt' using 1:5 with lines lw 2 title 'KBH Susceptible', \\\n");
    fprintf(pipe, "     'data_file.txt' using 1:6 with lines lw 2 title 'KBH Infected', \\\n");
    fprintf(pipe, "     'data_file.txt' using 1:7 with lines lw 2 title 'KBH Recovered'\n");
    fflush(pipe);
    _pclose(pipe);
}